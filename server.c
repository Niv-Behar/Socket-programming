...
uint8_t serverMakeChoice(char board[ROWS][COLUMNS]) {
    ...
    while(temp==0)
   	{//random choice for server
	row = (choice-1) / ROWS;
        column = (choice-1) % COLUMNS;
        if ((board[row][column] != CLIENT_MARK) && (board[row][column] != SERVER_MARK))
                return (uint8_t) choice;
	else choice = (rand() % (upper - lower + 1)) + lower;
    }
    return (uint8_t) choice;
}
...
void processBuffer(
    ...
    // gameType == END_GAME
    // check gameId, port & ip, and seqNum
    if (gameId != buffer[5]) {
...
void playServer(
    ...
    for (long j=0; j<LONG_MAX; j++) {
        ...
        for (int i=0; i<MAX_BOARD; i++) {
            if (FD_ISSET(boardInfo[i].sd, &socketFDS)) { 
